...
dBaseBuildConfigurationWidget::dBaseBuildConfigurationWidget(dBaseBuildConfiguration *bc)
    : NamedWidget(nullptr)
    , m_bc(bc)
{
    // Build UI
    ...
    connect(bc, &dBaseBuildConfiguration::buildDirectoryChanged,
            this, &dBaseBuildConfigurationWidget::updateUi);
    connect(m_buildDirectoryChooser, &PathChooser::pathChanged,
            this, &dBaseBuildConfigurationWidget::onPathEdited);

    setDisplayName(tr(Constants::C_DBASEBUILDCONFIGURATIONWIDGET_DISPLAY));
    updateUi();
}

dBaseBuildConfigurationWidget::~dBaseBuildConfigurationWidget() = default;

void dBaseBuildConfigurationWidget::updateUi()
{
    m_buildDirectoryChooser->setPath(m_bc->buildDirectory().toUserOutput());
}

void dBaseBuildConfigurationWidget::onPathEdited(const QString &path)
{
